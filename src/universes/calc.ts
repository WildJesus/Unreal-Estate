...
// ── Legacy shims (content.ts v0.2.x compatibility — remove in step 4) ─────────
// These delegate to the new primitives above so there is no duplicated logic.

/** @deprecated Use MODEL_DEFAULTS from data.ts directly */
export const DEFAULTS = {
  LTV:          1 - MODEL_DEFAULTS.downPaymentRatio,  // 0.90
  TERM_MONTHS:  MODEL_DEFAULTS.loanTermMonths,         // 360
} as const;

/** @deprecated Use annuityFactor() */
export function mortgagePaymentFactor(rateAnnual: number, termMonths: number): number {
  return annuityFactor(rateAnnual, termMonths);
}

/** @deprecated Use computeBurdenComparison() */
export function monthlyMortgagePayment(
  ...
  return price * ltv * annuityFactor(rateAnnual, termMonths);
}

/** @deprecated Use BurdenComparison.burdenEquivalentPrice */
export interface AdjustedPriceParams {
  currentPrice: number;
  houseIndexTarget: number;  houseIndexCurrent: number;
  incomeTarget: number;      incomeCurrent: number;
  targetMortgageRate: number; currentMortgageRate: number;
  termMonths: number;
}

/** @deprecated Use computeBurdenComparison() */
export function computeAdjustedPrice(p: AdjustedPriceParams): number {
  return (
    p.currentPrice *
    (p.houseIndexTarget  / p.houseIndexCurrent) *
    (p.incomeCurrent     / p.incomeTarget) *
    (annuityFactor(p.targetMortgageRate,  p.termMonths) /
     annuityFactor(p.currentMortgageRate, p.termMonths))
  );
}

/** @deprecated Use BurdenComparison directly */
